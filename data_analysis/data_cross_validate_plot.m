%% Load data
load('/home/sou/Documents/Data/dresults/xval_standard_stitching0_flipback0_collapsed1_RSPdRSPgSC_2021_03_08_18_15.mat')
...
used_dim = [1,1,1,0]; used_dim = used_dim(1:ndims(Xobs)-1);
...
observed_data2 = permute(observed_data(:,:,:,:,1:5),[1,2,4,3,5]);
N = size(observed_data2,1);
T = size(observed_data2,2);
C = size(observed_data2,3);
E = size(observed_data2,4);
K = size(observed_data2,5);


[smlty_tot,ref_tot,permt_tot,~,sig_tot]   = get_similarity_tot(models_tot,[1,1,1] ,1:size(models_tot,5), [1,0,1],reshape(observed_data2,[N,T,C*E*K]));
%%
...
%%
% Trials and conditions miht be are collapsed
    ...
    for modeli = 1:Nmodels
        ...
        for jj = 1:Nhyper2
            for kk = 1:Nhyper3
                ...
                % TO BE REMOVED ! 
                Dp = squeeze(median(VOIcur(modeli,:,jj,kk,:),5));
