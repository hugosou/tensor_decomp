function [Xobs,observed_data,true_params,Xtrue] = tmp_build_toydataset(model_true,Rtrue,Xdims,add_offset,add_missing)
...
Ttrue(:,1) = abs(sin(linspace(0,2*pi,D2))).^2;
Ttrue(:,2) = abs(sin(linspace(0,2*pi,D2))).^2;

% Neuron Loadings
Ntrue = normrnd(0,2,D1,Rtrue);

% Neuron Groups
neurons_groups = zeros(D1,2);
neurons_groups(:,1) = (1:D1) < D1/2;
neurons_groups(:,2) = (1:D1) >= D1/2;
CG =[sum(neurons_groups.*[0,1],2),sum(neurons_groups.*[1,0],2)];
Ntrue=Ntrue.*CG;
