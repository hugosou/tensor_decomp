function vi_var = vi_update_CP_old(vi_var,vi_param,Xobs)
...
tensor_mean   = reshape(CP_mean{1}*KhatriRaoProd_old(CP_mean{end:-1:2})', Xdims);
AAt = get_AAt(CP_mean,CP_variance);
tensor2_mean   = reshape(AAt{1}*KhatriRaoProd_old(AAt{end:-1:2})', Xdims);
...
if dimn == 1
    ...
    Ur = KhatriRaoProd_old(factors{N:-1:2});
    Y = reshape(tensor,szn,szr);
    MTTKRP =  Y * Ur;
    
elseif dimn == N
    % Fast to unfold 1st or last
    Ul = KhatriRaoProd_old(factors{N-1:-1:1});
    Y = reshape(tensor,szl,szn);
    MTTKRP = Y' * Ul;
    
else
    % Left and Right KhatriRao
    Ul = KhatriRaoProd_old(factors{N:-1:dimn+1});
    Ur = reshape(KhatriRaoProd_old(factors{dimn-1:-1:1}), szl, 1, R);
